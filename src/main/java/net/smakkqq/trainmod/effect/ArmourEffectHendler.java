...
public class ArmourEffectHendler implements ServerTickEvents.EndTick {

    private static final int EFFECT_REFRESH_INTERVAL = 80;

    private static final Map<TagKey<Item>, List<StatusEffectInstance>> EFFECT_MAP
	    = new ImmutableMap.Builder<TagKey<Item>, List<StatusEffectInstance>>()
		    .put(ModTags.Items.SAPPHIRE_ARMOR, List.of(
	...
	for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
	    if (hasFullSuitAmorOn(player)) {
		applyArmorEffects(player);
	    }
	}
    }

    private boolean hasFullSuitAmorOn(ServerPlayerEntity player) {
	...
	for (Map.Entry<TagKey<Item>, List<StatusEffectInstance>> entry : EFFECT_MAP.entrySet()) {
